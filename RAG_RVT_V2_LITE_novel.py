...
class V2_Lite_Fusion(nn.Module):
    ...
    def __init__(self, cnn_channels, vit_dim, embed_dim, num_heads=4, dropout=0.1, drop_path=0.1):
        ...
        # TRR-Lite: Token Reliability Routing (parameter-free)
        # Helps suppress noisy spatial tokens/patches before cross-attention.
        self.trr_eps = 1e-6

    @staticmethod
    def _token_reliability_weights(seq, eps=1e-6):
        """Compute per-token reliability weights in [0,1] using normalized L2 energy (no params).
        seq: (B, S, D)
        """
        # L2 energy per token
        e = torch.sqrt((seq * seq).sum(dim=-1) + eps)  # (B, S)
        # Normalize within each sample
        mu = e.mean(dim=1, keepdim=True)
        std = e.std(dim=1, keepdim=True).clamp_min(eps)
        z = (e - mu) / std
        # Sigmoid squashing -> reliability weights
        w = torch.sigmoid(z).unsqueeze(-1)  # (B, S, 1)
        return w

    @staticmethod
    def _cosine_gate(a, b, eps=1e-6):
        """Cosine-similarity-based gate in [0,1] (no params). a,b: (B,D)"""
        a_n = a / (a.norm(dim=1, keepdim=True).clamp_min(eps))
        b_n = b / (b.norm(dim=1, keepdim=True).clamp_min(eps))
        cos = (a_n * b_n).sum(dim=1, keepdim=True)  # (B,1) in [-1,1]
        return torch.sigmoid(cos)  # (B,1)
    ...
    def forward(self, cnn_features, vit_features):
        ...
        # TRR-Lite: token-wise reliability routing (parameter-free)
        cnn_w = self._token_reliability_weights(cnn_proj, eps=self.trr_eps)
        vit_w = self._token_reliability_weights(vit_proj, eps=self.trr_eps)
        cnn_proj = cnn_proj * cnn_w
        vit_proj = vit_proj * vit_w

        # ACA-Lite: asymmetric cross-attention scaling (parameter-free)
        cnn_mean = cnn_proj.mean(dim=1)
        vit_mean = vit_proj.mean(dim=1)
        sim_gate = self._cosine_gate(cnn_mean, vit_mean, eps=self.trr_eps)  # (B,1)
        ...
        # Apply asymmetric residual scaling based on modality agreement.
        # When modalities agree (sim_gate→1), allow more mutual exchange; otherwise keep stronger self-signal.
        cnn_attended = cnn_proj + sim_gate.unsqueeze(1) * (cnn_attended - cnn_proj)
        vit_attended = vit_proj + (1.0 - sim_gate).unsqueeze(1) * (vit_attended - vit_proj)
...
# -----------------------------------------------------------------------------------
class RAG_RVT_V2_Lite_novel(nn.Module):
    """

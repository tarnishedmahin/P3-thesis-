...
from RAG_RVT_V2_LITE_novel import RAG_RVT_V2_Lite_novel

# -----------------------------
# User paths
# -----------------------------
MODEL_TAG = "RAG_RVT_V2_LITE_novel"
...
class CFG:
    ...
    # Self-distillation (training-only, no extra parameters)
    ...
    for epoch in range(cfg.epochs):
        ...
        for imgs, labels in tr_loader:
            ...
            if cfg.use_auxiliary_loss:
                ...
                # Self-distillation consistency (training-only, parameter-free)
                T = cfg.distill_temp
                w = cfg.distill_weight
                with torch.no_grad():
                    p_final = F.softmax(final_logits / T, dim=1)
                kl_cnn = F.kl_div(F.log_softmax(cnn_logits / T, dim=1), p_final, reduction="batchmean")
                kl_vit = F.kl_div(F.log_softmax(vit_logits / T, dim=1), p_final, reduction="batchmean")
                distill_loss = (kl_cnn + kl_vit) * (T * T)

                loss = 0.5 * total_loss + 0.5 * final_loss + w * distill_loss
            else:
                logits = model(imgs)
                loss = criterion(logits, labels)
